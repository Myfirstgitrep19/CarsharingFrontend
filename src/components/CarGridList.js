...
export const CarGridList = (props) => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        ...
        const updatedJson = request.data.map(({ idCar: id, manufacturer :manuf, model :model1, color :color1,yofProd :yofProd1, kilometers :kilometers1, priceDay :priceDay1, isAvailable :isAvailable1, insurance :insurance1, segment : segment1, regNumbers :regNumbers1, filePath :filePath1, techRev :techRev1 }) => ({
          id,
          manuf,
          model1,
          color1,
    ...
    return ( 
      ...
      <div style={{ height: 600, marginTop : 80}}>
        <DataGrid rows={carList} columns={columns} pageSize={10} checkboxSelection id="idCar" />
